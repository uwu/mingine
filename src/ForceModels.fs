...
let weight (gravity: Vec2<m/s^2>) gObj (_: float<s>) (_: float<s>) : ForceAndTorque =
    // Fg=mg, no torque
...
let spring (springConstant: float<N/m>) restPos (connectionOset: Vec2<_>) gObj (_: float<s>) (_: float<s>) : ForceAndTorque =
    let distToCentre = gObj.pos - restPos
    let e = distToCentre + (connectionOset.rotate -gObj.angle Vec2.origin)
    // Fs=-ke
...
let simpleDamping (posRatio: float) (angRatio: float) gObj (_: float<s>) (_: float<s>) : ForceAndTorque =
    // this force isnt really a proper scientific calculation but oh well
    -gObj.velocity * posRatio |> Vec2.map Units.typedToTyped,
